...
protected:
    void SetUp() override {
        ...
        testData = readHeightGainTerminalCorrectionModelTestData("HeightGainTerminalCorrectionModelTestData.csv");
    }
...
TEST_F(HeightGainTerminalCorrectionModelTest, TestHeightGainTerminalCorrectionModel) {
    ...
    for (const auto& data : testData) {
        rtn = HeightGainTerminalCorrectionModel(data.f__ghz, data.h__meter, data.w_s__meter, data.R__meter, data.clutter_type, &A_h__db);
...
TEST(Section3p1_InputValidationTest, Section3p1_FrequencyInvalid) {
    EXPECT_EQ(Section3p1_InputValidation(0.02, 1, 1, 1), ERROR31__FREQUENCY);  // Frequency too low
    EXPECT_EQ(Section3p1_InputValidation(3.01, 1, 1, 1), ERROR31__FREQUENCY);  // Frequency too high
}

TEST(Section3p1_InputValidationTest, Section3p1_AntennaHeightInvalid) {
    EXPECT_EQ(Section3p1_InputValidation(1, -1, 1, 1), ERROR31__ANTENNA_HEIGHT); // Antenna height negative
    EXPECT_EQ(Section3p1_InputValidation(1, 0, 1, 1), ERROR31__ANTENNA_HEIGHT);  // Antenna height == 0
}

TEST(Section3p1_InputValidationTest, Section3p1_StreetWidthInvalid) {
    EXPECT_EQ(Section3p1_InputValidation(1, 1, -1, 1), ERROR31__STREET_WIDTH);  // Street width negative
    EXPECT_EQ(Section3p1_InputValidation(1, 1, 0, 1), ERROR31__STREET_WIDTH);   // Street width == 0
}

TEST(Section3p1_InputValidationTest, Section3p1_ClutterHeightInvalid) {
    EXPECT_EQ(Section3p1_InputValidation(1, 1, 1, -1), ERROR31__CLUTTER_HEIGHT);  // Clutter height negative
    EXPECT_EQ(Section3p1_InputValidation(1, 1, 1, 0), ERROR31__CLUTTER_HEIGHT);   // Clutter height == 0
}
...
TEST(Section3p1_InputValidationTest, Section3p1_ValidInputs) {
    ...
    EXPECT_EQ(Section3p1_InputValidation(1, std::numeric_limits<double>::max(), 1, 1), SUCCESS);
    EXPECT_EQ(Section3p1_InputValidation(1, 1, std::numeric_limits<double>::max(), 1), SUCCESS);
    EXPECT_EQ(Section3p1_InputValidation(1, 1, 1, std::numeric_limits<double>::max()), SUCCESS);
}
...
TEST(Section3p1_Equation_2aTest, Section3p1_NuAboveLimit) {
    // Test a few values with separately-computed expected results for this equation

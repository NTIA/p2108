...
protected:
    void SetUp() override {
        ...
        testData = readTerrestrialStatisticalModelTestData("TerrestrialStatisticalModelTestData.csv");
    }
...
TEST_F(TerrestrialStatisticalModelTest, TestTerrestrialStatisticalModel) {
    ...
    for (const auto& data : testData) {
        rtn = TerrestrialStatisticalModel(data.f__ghz, data.d__km, data.p, &L_ctt__db);
...
TEST(Section3p2_InputValidationTest, Section3p2_FrequencyInvalid) {
    EXPECT_EQ(Section3p2_InputValidation(0.49, 1, 1), ERROR32__FREQUENCY);   // Frequency too low
    EXPECT_EQ(Section3p2_InputValidation(67.01, 1, 1), ERROR32__FREQUENCY);  // Frequency too high
}

TEST(Section3p2_InputValidationTest, Section3p2_DistanceInvalid) {
    EXPECT_EQ(Section3p2_InputValidation(20, 0.249, 1), ERROR32__DISTANCE); // Distance too small
    EXPECT_EQ(Section3p2_InputValidation(20, 0, 1), ERROR32__DISTANCE);     // Distance is zero
    EXPECT_EQ(Section3p2_InputValidation(20, -1, 1), ERROR32__DISTANCE);    // Distance is negative
}

TEST(Section3p2_InputValidationTest, Section3p2_PercentageInvalid) {
    EXPECT_EQ(Section3p2_InputValidation(20, 1, -1), ERROR32__PERCENTAGE);  // p < 0
    EXPECT_EQ(Section3p2_InputValidation(20, 1, 0), ERROR32__PERCENTAGE);   // p = 0
    EXPECT_EQ(Section3p2_InputValidation(20, 1, 100), ERROR32__PERCENTAGE); // p = 100
    EXPECT_EQ(Section3p2_InputValidation(20, 1, 101), ERROR32__PERCENTAGE); // p > 100
}

...
protected:
    void SetUp() override {
        ...
        testData = readAeronauticalStatisticalModelTestData("AeronauticalStatisticalModelTestData.csv");
    }
...
TEST_F(AeronauticalStatisticalModelTest, TestAeronauticalStatisticalModel) {
    ...
    for (const auto& data : testData) {
        rtn = AeronauticalStatisticalModel(data.f__ghz, data.theta__deg, data.p, &L_ces__db);
...
TEST(Section3p3_InputValidationTest, Section3p3_FrequencyInvalid) {
    EXPECT_EQ(Section3p3_InputValidation(9, 1, 1), ERROR33__FREQUENCY);   // Frequency too low
    EXPECT_EQ(Section3p3_InputValidation(101, 1, 1), ERROR33__FREQUENCY); // Frequency too high
}

TEST(Section3p3_InputValidationTest, Section3p3_ThetaInvalid) {
    EXPECT_EQ(Section3p3_InputValidation(20, -1, 1), ERROR33__THETA); // Theta negative
    EXPECT_EQ(Section3p3_InputValidation(20, 91, 1), ERROR33__THETA); // Theta too large
}

TEST(Section3p3_InputValidationTest, Section3p3_PercentageInvalid) {
    EXPECT_EQ(Section3p3_InputValidation(20, 1, -1), ERROR33__PERCENTAGE);  // p < 0
    EXPECT_EQ(Section3p3_InputValidation(20, 1, 0), ERROR33__PERCENTAGE);   // p = 0
    EXPECT_EQ(Section3p3_InputValidation(20, 1, 100), ERROR33__PERCENTAGE); // p = 100
    EXPECT_EQ(Section3p3_InputValidation(20, 1, 101), ERROR33__PERCENTAGE); // p > 100
}

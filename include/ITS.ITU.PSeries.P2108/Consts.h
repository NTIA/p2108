#ifndef _P2108_CONSTS_
#define _P2108_CONSTS_

///////////////////////////////////////////////
// CONSTANTS
//

// Clutter Types
#define CLUTTER_TYPE__WATER_SEA                 1
#define CLUTTER_TYPE__OPEN_RURAL                2
#define CLUTTER_TYPE__SUBURBAN                  3
#define CLUTTER_TYPE__URBAN                     4
#define CLUTTER_TYPE__TREES_FOREST              5
#define CLUTTER_TYPE__DENSE_URBAN               6

// Default Clutter Heights
#define DEFAULT_CLUTTER_HEIGHT__WATER_SEA       10
#define DEFAULT_CLUTTER_HEIGHT__OPEN_RURAL      10
#define DEFAULT_CLUTTER_HEIGHT__SUBURBAN        10
#define DEFAULT_CLUTTER_HEIGHT__URBAN           15
#define DEFAULT_CLUTTER_HEIGHT__TREES_FOREST    15
#define DEFAULT_CLUTTER_HEIGHT__DENSE_URBAN     20

#endif